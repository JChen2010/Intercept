...
void test_monitoring(int syscall) {
  printf("whitelist only:\n");
  do_intercept(syscall, 0);
  do_whitelist(syscall);
  do_release(syscall, 0);

  printf("blacklist only:\n");
  do_intercept(syscall, 0);
  do_blacklist(syscall);
  do_release(syscall, 0);

  printf("whitelist to blacklist:\n");
  do_intercept(syscall, 0);
  do_from_white_to_black(syscall);
  do_release(syscall, 0);
}

int do_whitelist(int syscall) {
  // Start monitoring specific PIDs (Whitelist mode)
  do_start(syscall, 1, 0);
  do_start(syscall, 3, 0);
  do_start(syscall, 5, 0);

  // Should not be able to stop the PIDs which are not on the whitelist
  do_stop(syscall, 7, -EINVAL);
  do_stop(syscall, 9, -EINVAL);

  // Should be able to stop the PIDs which are on the whitelist
  do_stop(syscall, 1, 0);
  do_stop(syscall, 3, 0);
  do_stop(syscall, 5, 0);
...
int do_blacklist(int syscall) {
  // Start monitoring all PIDs (Blacklist mode)
  do_start(syscall, 0, 0);

  // Should not be able to start the PIDs that are not on the blacklist
  do_start(syscall, 7, -EBUSY);
  do_start(syscall, 9, -EBUSY);

  // Should be able to add specific PIDs to the blacklist
  do_stop(syscall, 1, 0);
  do_stop(syscall, 3, 0);
  do_stop(syscall, 5, 0);

  // Should be able to remove the PIDs which are on the blacklist
  do_start(syscall, 1, 0);
  do_start(syscall, 3, 0);
  do_start(syscall, 5, 0);
...
int do_from_white_to_black(int syscall) {
  // Start monitoring specific PIDs (Whitelist mode)
  do_start(syscall, 1, 0);
  do_start(syscall, 3, 0);
  do_start(syscall, 5, 0);
  ...
  // Should not be able to add the same PIDs to the blacklist
  do_start(syscall, 1, -EBUSY);
  do_start(syscall, 3, -EBUSY);
  do_start(syscall, 5, -EBUSY);
  // Should not be able to add these new PIDs to the blacklist
  do_start(syscall, 2, -EBUSY);
  do_start(syscall, 4, -EBUSY);
  do_start(syscall, 6, -EBUSY);
  // Can still stop these PIDs since we are monitoring all
  do_stop(syscall, 1, 0);
  do_stop(syscall, 3, 0);
  do_stop(syscall, 5, 0);
